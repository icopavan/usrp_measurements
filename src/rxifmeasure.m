...
for i = 1:length(gains)
    ...
    for j = 1:length(cfreqs)
        ...
        for k = 1:length(ifreqs8)
            ...
            status = 1;
            while status ~= 0
                [status, p, y] = rxifsingle(50e6, 8, gains(i), cfreqs(j), ifreqs8(k));
                if status ~= 0
                    fprintf(1, 'Error %d!\n', status);
                end
            end
            ...
            ys8(i, j, k) = y;
            fprintf(1, '%gdB %g\n', p, y);
            save(savefile, 'gains', 'cfreqs', 'ifreqs8', 'ifreqs16', 'ys8', 'ys16', 'ps8', 'ps16');
        end
        toc
        ...
        for k = 1:length(ifreqs16)
            ...
            status = 1;
            while status ~= 0
                [status, p, y] = rxifsingle(25e6, 16, gains(i), cfreqs(j), ifreqs16(k));
                if status ~= 0
                    fprintf(1, 'Error %d!\n', status);
                end
            end
