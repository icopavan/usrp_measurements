...
every measurement setup uses dedicated network connection for the measurement
devices and the \gls{usrp}.

``Part A''and ``Part B'' in the following sections refer to \cref{fig:usrp}.
...
According to the datasheet of the DEMOD (see \cref{fig:usrp}) \cite{demod}, it
causes a part of the ripple seen in \cref{fig:gainfscrf}. The rest of the ripple is
caused by mismatch of the individual components.
...
a \SI{16}{\deci\bel} attenuator, to Since the power head can only measure signals between \SI{-60}{\deci\belm} and
\SI{20}{\deci\belm} a \SI{6}{\deci\bel} and a \SI{10}{\deci\bel} attenuator were used,
to bring down the signal to acceptable levels.
\begin{figure}[htb]
    \centering
\begin{tikzpicture}
    \begin{axis}[
