...
are characterized by the intermodulation properties. These results
...
only added feature is a flag which enables asynchronously sending samples. This
...
set to $\SI{-10}{\deci\belm}-gain$ output power. For calibration, a power meter was used in place of the \gls{usrp} and the measurement
data corrected as if the $\SI{-10}{\deci\belm}-gain$ were present at the input port.
...
the used vector generator (see \cref{sec:meters}).
...
and with \SI{16}{\bit} per sample. Measured samples were corrected by a second measurement with a power meter in place of the
...
gain setting of \SI{31.5}{\deci\bel}, while stepping the signal power. This also means,
that the intermodulation products shown in \cref{fig:rxip3} are
...
Both $IM_3$ and $out$ were fitted to curves according to the formula $3\cdot{}x+a$ resp.
$1\cdot{}x+a$. The point were both meet is the third order intercept point. The calculated
values are: third order input intercept point $IIP_3 = \SI{-7.6}{\deci\belm}$ and
third order output intercept point $OIP_3 = \SI{25.5}{\deci\belfs}$.
...
and with \SI{16}{\bit} per sample. Measured samples were corrected by the forward transfer function of the cable with the
...
and an additional amplifier is needed for high power applications.
