...
them into the file named {\ttfamily filename} ({\ttfamily File Sink}). {\ttfamily Head}
...
\cref{lst:rxfinal.py}, which has been generated from the flowgraph in \cref{fig:grc_rx}
and modified to support additional features.

All the scripts contain a class (\cref{line:classdef}) which represents the flowgraph
...
stop processing early. Additional information on the particular modifications
can be found in \cref{sec:measgnu}.
...
Since the USRP GNU Radio Companion module has no \gls{gui} parameter to turn off DC offset
