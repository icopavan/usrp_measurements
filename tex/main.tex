...
\usepackage{tikz}
\usetikzlibrary{dsp,chains,fit,matrix,positioning,scopes,calc,backgrounds,arrows,decorations.pathmorphing}
...
\begin{abstract}
    ...
    there is no data available on the RF characteristics of USRP devices.
\end{abstract}

\section{Introduction}
Software defined radio systems (SDR) can be operated on a very broad frequency range,
easily adapted to various needs and allow operations which would be
impossible or very expensive in analog hardware. A SDR consists of specialised
software, a general purpose computer, and an analog frontend with fast
ADCs/DACs and optionally DSP and mixing capabilities. One of those frontends is
the Ettus Research USRP N2x0 product family. This family consists of the N200 and
the N210, of which the latter has a bigger FPGA, allowing additional custom DSP code directly
...
from DC to 6 GHz\cite{ettus_n2x0}. These products are also available as rebranded
versions from NI.
...
\begin{tikzpicture}[node distance=0.6]
    ...
    \draw node[rotate=90,anchor=south] at (lp6.north) {LP1};
...
gives a more in depth description of the analog frontend and the FPGA board.
...
\cite{ni_29xx} only. It can also be seen in \cref{sec:iftx,sec:ifrx}.


It is advisable to always use the latest driver versions, which are not necessarily included in
the most recent Linux distributions. Changes to byte ordering in the protocol \cite{usrp_byte} can
...
\Cref{sec:dsprx} and \cref{sec:dsptx} explain both in more detail, which have been derived from the FPGA sources at \cite{usrp_src}.
In order to minimize noise, both chains use round to nearest with error diffusion and clipping
for converting signals to lower bit widths. IQ samples are generated with a final bit width of
...
\begin{tikzpicture}
    \matrix (m1) [row sep=5mm, column sep=5mm,ampersand replacement=\&]
    {
        ...
        \node[coordinate]                           (m110){};       \&
        \node[dspsquare,inner sep=3pt]              (m111){CIC};    \&
        ...
        \node[coordinate]                           (m27) {};       \&
        \node[dspswitch]                            (m28) {};       \&
        \node[dspsquare,inner sep=3pt]              (m29) {cordic}; \&
        \node[coordinate]                           (m210) {};   \&
        \node[coordinate]                           (m211) {};      \&
        \node[coordinate]                           (m212) {};      \&
        \node[coordinate]                           (m213) {};      \&
        ...
        \node[coordinate]                           (m39) {};       \&
        \node[coordinate]                           (m310){};       \&
        \node[dspsquare,inner sep=3pt]              (m311){CIC};    \&
